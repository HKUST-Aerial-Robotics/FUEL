...
public:
  ...
  typedef explicit_error_stepper_base<Stepper, Order, StepperOrder, ErrorOrder, State, Value, Deriv,
  ...
  do_step(System system, const StateIn& in, time_type t, StateOut& out, time_type dt) {
    ...
    m_resizer.adjust_size(in, detail::bind(&internal_stepper_base_type::template resize_impl<StateIn>,
  ...
  typename boost::disable_if<boost::is_same<DerivIn, time_type>, void>::type do_step(
      System system, const StateIn& in, const DerivIn& dxdt, time_type t, StateOut& out, time_type dt) {
  ...
  void do_step(System system, const StateIn& in, const DerivIn& dxdt, time_type t, StateOut& out,
...
private:
  ...
  void do_step_v1(System system, StateInOut& x, time_type t, time_type dt) {
    typename odeint::unwrap_reference<System>::type& sys = system;
    m_resizer.adjust_size(x, detail::bind(&internal_stepper_base_type::template resize_impl<StateInOut>,
                                          detail::ref(*this), detail::_1));
    sys(x, m_dxdt.m_v, t);
 ...
 *
 * This class serves as the base class for all explicit steppers with algebra and operations. In contrast
 * to
 ...
 *
 * explicit_error_stepper_base provides several overloaded `do_step` methods, see the list below. Only
 * two of them
 * are needed to fulfill the Error Stepper concept. The other ones are for convenience and for
 * performance. Some
 * of them simply update the state out-of-place, while other expect that the first derivative at `t` is
 * passed to the
 * stepper.
 *
 ...
 * - `do_step( sys , x , dxdt , t , dt )` - This method updates the state in-place, but the derivative at
 * the point `t`
 *      must be explicitly passed in `dxdt`.
 * - `do_step( sys , in , t , out , dt )` - This method updates the state out-of-place, hence the result
 * of the step
 *      is stored in `out`.
 * - `do_step( sys , in , dxdt , t , out , dt )` - This method update the state out-of-place and expects
 * that the
 *     derivative at the point `t` is explicitly passed in `dxdt`. It is a combination of the two
 * `do_step` methods
 *     above.
 ...
 * be used for x.
 * - `do_step( sys , x , dxdt , t , dt , xerr )` - This method updates the state in-place, but the
 * derivative at the
 ...
 * estimates the error
 *      during the step.
 * - `do_step( sys , in , dxdt , t , out , dt , xerr )` - This methods updates the state out-of-place and
 * estimates
...
/**
 * \fn explicit_error_stepper_base::explicit_error_stepper_base( const algebra_type &algebra =
 * algebra_type() )
 *
...
/**
 * \fn explicit_error_stepper_base::do_step( System system , StateInOut &x , const DerivIn &dxdt ,
...
/**
 * \fn explicit_error_stepper_base::do_step( System system , const StateIn &in , time_type t , StateOut
 * &out , time_type dt )
...
/**
 * \fn explicit_error_stepper_base::do_step( System system , const StateIn &in , const DerivIn &dxdt ,
 * time_type t , StateOut &out , time_type dt )
 * \brief The method performs one step with the stepper passed by Stepper. The state of the ODE is
 * updated out-of-place.
 * Furthermore, the derivative of x at t is passed to the stepper. It is supposed to be used in the
 * following way:
 *
 * \code
 * sys( in , dxdt , t );
 ...
 * \endcode
 *
 * The result is updated in place in x. This method is disabled if Time and DerivIn are of the same type.
 * In this
 * case the method could not be distinguished from other `do_step` versions.
 *
 * \note This method does not solve the forwarding problem.
 ...
 * \brief The method performs one step with the stepper passed by Stepper. The state of the ODE is
 * updated out-of-place.
 * Furthermore, the derivative of x at t is passed to the stepper and the error is estimated. It is
 * supposed to be used in the following way:
 *
 * \code
 * sys( in , dxdt , t );
 * stepper.do_step( sys , in , dxdt , t , out , dt );
