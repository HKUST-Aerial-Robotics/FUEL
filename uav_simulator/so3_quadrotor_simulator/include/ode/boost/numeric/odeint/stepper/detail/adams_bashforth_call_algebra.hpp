...
struct adams_bashforth_call_algebra<1, Algebra, Operations> {
  ...
  void operator()(Algebra& algebra, const StateIn& in, StateOut& out, const StepStorage& steps,
                  const Coefficients& coef, Time dt) const {
    typedef typename Coefficients::value_type value_type;
    algebra.for_each3(out, in, steps[0].m_v,
                      typename Operations::template scale_sum2<value_type, Time>(1.0, dt * coef[0]));
  }
};
...
struct adams_bashforth_call_algebra<2, Algebra, Operations> {
  ...
  void operator()(Algebra& algebra, const StateIn& in, StateOut& out, const StepStorage& steps,
    ...
    algebra.for_each4(out, in, steps[0].m_v, steps[1].m_v,
                      typename Operations::template scale_sum3<value_type, Time, Time>(1.0, dt * coef[0],
                                                                                       dt * coef[1]));
  }
};
...
struct adams_bashforth_call_algebra<3, Algebra, Operations> {
  ...
  void operator()(Algebra& algebra, const StateIn& in, StateOut& out, const StepStorage& steps,
    ...
    algebra.for_each5(out, in, steps[0].m_v, steps[1].m_v, steps[2].m_v,
                      typename Operations::template scale_sum4<value_type, Time, Time, Time>(
                          1.0, dt * coef[0], dt * coef[1], dt * coef[2]));
  }
};
...
struct adams_bashforth_call_algebra<6, Algebra, Operations> {
  ...
  void operator()(Algebra& algebra, const StateIn& in, StateOut& out, const StepStorage& steps,
    ...
    typedef typename Coefficients::value_type value_type;
    algebra.for_each8(
        out, in, steps[0].m_v, steps[1].m_v, steps[2].m_v, steps[3].m_v, steps[4].m_v, steps[5].m_v,
...
struct adams_bashforth_call_algebra<7, Algebra, Operations> {
  ...
  void operator()(Algebra& algebra, const StateIn& in, StateOut& out, const StepStorage& steps,
    ...
    BOOST_ASSERT(false);  // not implemented
                          // typedef typename Coefficients::value_type value_type;
    // Algebra::for_each9( out , in , steps[0] , steps[1] , steps[2] , steps[3] , steps[4] , steps[5] ,
    // steps[6]
    //                     typename Operations::template scale_sum8< value_type , Time , Time , Time ,
    //                     Time , Time , Time >(
    //                         1.0 , dt * coef[0] , dt * coef[1] , dt * coef[2] , dt * coef[3] , dt *
    //                         coef[4] , dt * coef[5] , dt * coef[6] ) );
  }
};
...
struct adams_bashforth_call_algebra<8, Algebra, Operations> {
  ...
  void operator()(Algebra& algebra, const StateIn& in, StateOut& out, const StepStorage& steps,
    ...
    BOOST_ASSERT(false);  // not implemented
                          // typedef typename Coefficients::value_type value_type;
    // Algebra::for_each10( out , in , steps[0] , steps[1] , steps[2] , steps[3] , steps[4] , steps[5] ,
    // steps[6] , steps[7] ,
    //                      typename Operations::template scale_sum9< value_type , Time , Time , Time ,
    //                      Time , Time , Time , Time >(
    //                          1.0 , dt * coef[0] , dt * coef[1] , dt * coef[2] , dt * coef[3] , dt *
    //                          coef[4] , dt * coef[5] , dt * coef[6] , dt * coef[7] ) );
  }
};
