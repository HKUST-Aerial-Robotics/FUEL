...
BOOST_AUTO_TEST_CASE(for_each2) {
  fusion_fixture f;
  algebra.for_each2(f.res, f.k1, default_operations::scale_sum1<time_type>(dt));
...
BOOST_AUTO_TEST_CASE(for_each3) {
  fusion_fixture f;
  algebra.for_each3(f.res, f.x, f.k1, default_operations::scale_sum2<value_type, time_type>(1.0, dt));
